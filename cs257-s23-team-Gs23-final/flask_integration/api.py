...
class CrimeDataAPI:
    ...
    def get_crime_rate_and_ranking_for(self, location):
        ...
        if location in states:
            state = location     
            query = "SELECT state, crime_rate, ranking FROM (SELECT state, crime_rate, RANK() OVER (ORDER BY crime_rate DESC) AS ranking FROM (SELECT state, AVG(total_offenses*1.0/population) AS crime_rate FROM national_offenses GROUP BY state ) AS state_crime_rates) AS ranked_states WHERE state = %s;"
        
        else:
            is_city = True  
            city, state = location.split(", ")
            query = "SELECT city, state, crime_rate, percentile_ranking FROM ( SELECT city, state, crime_rate, PERCENT_RANK() OVER (ORDER BY crime_rate ASC) AS percentile_ranking FROM ( SELECT city, state, total_offenses, population, total_offenses * 1.0 / population AS crime_rate FROM national_offenses) AS city_crime_data) AS filtered_data WHERE city = %s AND state = %s ; "
        try:
            cursor = self.connection.cursor()
            if is_city:
    ...
    def proportion_of_crime_type_committed(self, crime_type, location):
        ...
        if location == 'US': 
            ...
            city, state = location.split(", ")
            query = f"SELECT ROUND(SUM({crime_type} *1.0) / SUM(Total_Offenses *1.0),2) FROM national_offenses WHERE state= %s AND city = %s"
